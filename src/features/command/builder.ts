...
export function buildCommand(config: GlobalConfig): string {
    ...
    if (config.features.playlist.enabled) {
        if (config.features.playlist.items) {
            ...
            if (config.features.playlist.startIndex > 1) {
                parts.push(
                    `--playlist-start ${config.features.playlist.startIndex}`
                )
            }
            if (config.features.playlist.endIndex) {
                parts.push(
                    `--playlist-end ${config.features.playlist.endIndex}`
                )
            }
        }
    } else {
    ...
    if (config.features.postProcess.enabled) {
        if (config.features.postProcess.embedThumbnail)
            parts.push('--embed-thumbnail')
        if (config.features.postProcess.embedMetadata)
        ...
        if (config.features.postProcess.embedSubs) {
            ...
            if (config.features.postProcess.subtitleLangs) {
                parts.push(
                    `--sub-langs "${config.features.postProcess.subtitleLangs}"`
                )
            }
        }
        if (config.features.postProcess.proxy)
            parts.push(`--proxy "${config.features.postProcess.proxy}"`)
    }
