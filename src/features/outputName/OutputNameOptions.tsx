      ...
      <div className="flex flex-col gap-4">
        <label className="flex items-center gap-2 cursor-pointer">
           <input 
             ...
             onChange={(e) => toggleFeature('outputName', e.target.checked)}
        ...
        <div>
            ...
            <input 
               ...
               onChange={(e) => updateFeature('outputName', { name: e.target.value })}
               disabled={sections.enabled}
            />
            <p className="text-xs text-text-muted/70 mt-1">
              Will be saved as <code>{outputName.name || 'name'}.{ext}</code>
            </p>
        </div>
      </div>
    </Card>
